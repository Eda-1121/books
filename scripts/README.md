...
```
E:\Books\
├── paddle_output\   # PaddleOCR出力・作業フォルダ
└── epub\            # 完成EPUBの保存先
```
...
```bash
paddleocr pp_structurev3 -i "E:\Books\{本の名前}.pdf" \
  --use_doc_orientation_classify False \
  --use_doc_unwarping False \
  --save_path "E:\Books\paddle_output"
```

実行後、`paddle_output` に `{本の名前}_0.md`, `{本の名前}_1.md` ... が生成されます。
...
**処理内容：**
- ページごとのMDを番号順に結合 → `{本の名前}_combined.md` を生成
- 存在しない画像参照を削除
...
| `--paddle` | `E:\Books\paddle_output` | PaddleOCR出力フォルダ |
...
| `--lang` | `zh-CN` | 言語コード |
| `--paddle` | `E:\Books\paddle_output` | 作業フォルダ |
| `--epub` | `E:\Books\epub` | EPUB保存先フォルダ |
...
```bash
# Step 1: OCR
paddleocr pp_structurev3 -i "E:\Books\易经.pdf" \
  --use_doc_orientation_classify False \
  --use_doc_unwarping False \
  --save_path "E:\Books\paddle_output"
...
Step 2 完了後、`paddle_output\{本の名前}_combined.md` を直接編集してから Step 3 を実行してください。
...
```
{本の名前}_0.md ~ _N.md   # PaddleOCR生成（Step1）
    ↓ clean_and_combine.py
{本の名前}_combined.md     # 結合済み（Step2）
    ↓ fix_and_build.py
{本の名前}_fixed.md        # 整形済み（Step3中間）
    ↓ pandoc
{本の名前}.epub            # 完成（Step3）
```
